...
const char * print_extensions(){
	return glXQueryExtensionsString(dpy, DefaultScreen( dpy ));
}
...
void (*gIdle)(void);
void (*gKey)(unsigned int key, int x, int y,int state);
void (*gKeyUp)(unsigned char key, int x, int y,int state);
...
void glutKeyboardFunc(void (*func)(unsigned int key, int x, int y,int state))
{
	gKey = func;
}
void glutKeyboardUpFunc(void (*func)(unsigned char key, int x, int y,int state))
{
	gKeyUp = func;
}
...
void print_buttons(){
	...
	for(unsigned int i=0;i<10;i++){
		printf(" %i ",gButtonPressed[i]);
	}
	printf("\n");
}
int get_mouse_button(){
	for(unsigned int i=0;i<4;i++){
		if (gButtonPressed[i]!=0){
			return i;
		}
	}
	return 0;
...
char get_keysym(int key){
	//int temp1=1;
	//KeySym* temp= XGetKeyboardMapping(dpy,key,1,&temp1);
	return 'a';
