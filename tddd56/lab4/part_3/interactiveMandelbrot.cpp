...
// Image data
	...
	__global__
	void calculate_pixels(float *ptr)
	{
		int maxiter = 20;
		struct cuComplex
		{
	    ...
	    float jx = scale * (float)(gImageWidth/2 - x + offsetx/scale)/(gImageWidth/2);
	    float jy = scale * (float)(gImageHeight/2 - y + offsety/scale)/(gImageWidth/2);
