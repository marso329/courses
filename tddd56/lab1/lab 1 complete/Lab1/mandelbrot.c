...
static void compute_chunk(struct mandelbrot_param *args) {
	int i, j, val;
	float Cim, Cre;
	color_t pixel;
	...
	for (i = args->begin_h; i < args->end_h; i++) {
		...
		for (j = args->begin_w; j < args->end_w; j++) {
			...
			pixel = val > args->maxiter ?
					args->mandelbrot_color : color[val % num_colors(args)];
...
void init_ppm(struct mandelbrot_param* param) {
	...
	param->picture->data = malloc(
			ppm_align(sizeof(color_t) * param->width, PPM_ALIGNMENT)
					* param->height);
...
void update_colors(struct mandelbrot_param* param) {
	...
	for (i = 0; i < num_colors(param); i++) {
		color[i].green = (stop.green - start.green)
				* ((double) i / num_colors(param)) + start.green;
		color[i].red = (stop.red - start.red) * ((double) i / num_colors(param))
				+ start.red;
		color[i].blue = (stop.blue - start.blue)
				* ((double) i / num_colors(param)) + start.blue;
	}
}
